"""
FINN Dataflow Build for QAT CustomSmallNet -> Ultra96/KV260 bitstream.

Based on build_resnet18.py with CustomSmallNet-specific steps.

Usage (estimates only):
    python src/finn_build/build_custom_net.py --estimates-only

Usage (stop after a specific step for Netron inspection):
    python src/finn_build/build_custom_net.py --stop-after step_custom_net_streamline

Usage (full bitstream):
    python src/finn_build/build_custom_net.py
...
parser = argparse.ArgumentParser(
    description="FINN dataflow build for QAT CustomSmallNet."
)
parser.add_argument("--onnx", default="models/custom_net_m3_8w8a.onnx")
parser.add_argument("--output-dir", default="./build_finn_custom_net")
...
parser.add_argument("--board", default="Ultra96",
                    help="Target board (default: Ultra96)")
...
try:
    ...
    from custom_steps_custom_net import (
        step_custom_net_streamline,
        step_custom_net_lower,
        step_custom_net_to_hw,
    )
...
# ---------------------------------------------------------------------------
print(f"\n{'=' * 60}")
print(f"  FINN Dataflow Build - CustomSmallNet")
print(f"{'=' * 60}")
