"""
FINN Dataflow Build for QAT MobileNetV1 -> Ultra96-v2 / Kria KV260 bitstream.

Based on: ignore/finn-examples-rn18/build/mobilenet-v1/build.py

Key differences from finn-examples:
  - Target: Ultra96-v2 or Kria KV260 (Zynq) instead of ZCU104/Alveo
  - Model: 8w8a fundus classifier (4 classes, 512x512 input)
  - Our ONNX is raw QONNX — pipeline includes step_qonnx_to_finn + step_tidy_up
    (finn-examples starts from a pre-tidied model)
  - Preprocessing includes ImageNet normalization (FundusPreProc from ResNet18 build)
  - Uses target_fps_parallelization instead of manual folding config by default
  - No SLR floorplanning (Zynq, not Alveo)

Usage (estimates only):
    python src/finn_build/build_mobilenetv1.py --estimates-only

Usage (stop after a specific step for Netron inspection):
    python src/finn_build/build_mobilenetv1.py --stop-after step_tidy_up
    python src/finn_build/build_mobilenetv1.py --stop-after step_mobilenet_streamline
    python src/finn_build/build_mobilenetv1.py --stop-after step_mobilenet_lower_convs
    python src/finn_build/build_mobilenetv1.py --stop-after step_mobilenet_convert_to_hw

Usage (full bitstream):
    python src/finn_build/build_mobilenetv1.py
...
parser = argparse.ArgumentParser(
    description="FINN dataflow build for QAT MobileNetV1 -> Ultra96-v2 / Kria KV260."
)
parser.add_argument("--onnx", default="models/mobilenet_8w8a.onnx")
parser.add_argument("--output-dir", default="./build_finn_mobilenetv1")
...
try:
    ...
    from custom_steps_mobilenetv1 import (
        step_mobilenet_streamline,
        step_mobilenet_lower_convs,
        step_mobilenet_convert_to_hw,
    )
...
estimate_steps = [
    ...
    # Folding step inserted below based on CLI flag.
...
# ---------------------------------------------------------------------------
# Insert the correct folding step
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
print(f"\n{'=' * 60}")
print(f"  FINN Dataflow Build - Fundus QAT MobileNetV1")
print(f"{'=' * 60}")
