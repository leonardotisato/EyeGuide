...
from utils.dataset import train_transform_class, test_transform_class
...
def evaluate_zoom_levels(cfg, n_splits: int = 5, random_seed: int = 42) -> Dict[Any, Tuple[float, float]]:
    ...
    # se vuoi usare solo train+val definiti da prepare_dataframes:
    ...
    # ----------------- loop sui livelli di zoom -----------------
    for ext in zoom_levels:
        print(f"\n=== Zoom Level: {ext} ===")
        summary_log = f'./results/zoom_evaluation_{exp_name}_{ext}.csv'

        # CSV di riepilogo: una riga per fold per ogni zoom (miglior modello del fold)
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
            ...
            # --------- prepara dataset per questo fold ---------
            ...
            # --------- modello per questo fold ---------
            model = ResNet18Classifier(num_classes=cfg.nr_classes)
