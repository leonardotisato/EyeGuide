...
def qat_train_one_epoch(model, train_loader, optimizer, criterion, device, epoch, bn_freeze=BN_FREEZE_EPOCH):
    model.train()
    if epoch >= bn_freeze:
        freeze_bn(model)
...
def main(cfg: DictConfig) -> None:
    ...
    lr = float(OmegaConf.select(cfg, "qat_lr", default=QAT_LR))
    bn_freeze = int(OmegaConf.select(cfg, "bn_freeze", default=BN_FREEZE_EPOCH))
    patience = int(OmegaConf.select(cfg, "patience", default=PATIENCE))
    ...
    print(f"LR={lr}, BN freeze epoch={bn_freeze}, patience={patience}")

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=QAT_WEIGHT_DECAY)
    ...
    for epoch in tqdm(range(QAT_EPOCHS), desc="QAT Epochs"):
        train_loss, train_acc, train_f1 = qat_train_one_epoch(
            model, train_loader, optimizer, criterion, device, epoch, bn_freeze
        )
