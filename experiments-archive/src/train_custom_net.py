"""
Canonical FP32 KD fine-tuning of the revived CustomSmallNet branch.

This script aligns the archived custom-net experiment with the active project
pipeline:
- teacher: upgraded ResNet18 KD teacher (`resnet18_from_resnet50_fp32_kd.pth`)
- student: 224x224 CustomSmallNet
- teacher supervision: 512x512 full-image KD domain
- train loss: alpha * CE + (1 - alpha) * KL
- validation loss: same composite KD objective
- checkpoint selection: best val_loss

It produces the FP32 checkpoint consumed by `qat_custom_net.py`:
    models/custom_net_m3_fp32_kd.pth
"""
...
STUDENT_INPUT_SIZE = 224
TEACHER_INPUT_SIZE = 512
MULTIPLIER = 3
MODEL_NAME = f"custom_net_m{MULTIPLIER}"

student_test_transform = make_test_transform(STUDENT_INPUT_SIZE)
student_train_transform = make_strong_train_transform(STUDENT_INPUT_SIZE)
teacher_train_transform = make_strong_train_transform(TEACHER_INPUT_SIZE)
teacher_test_transform = make_test_transform(TEACHER_INPUT_SIZE)
...
class DualResDataset(Dataset):
    """Return (student_image, teacher_image, label) with branch-specific transforms."""
...
def main(cfg: DictConfig) -> None:
    ...
    print("Teacher loaded and frozen (512x512 full-image strong-train / clean-eval domain).")

    print(f"\n{'=' * 50}")
    print(f"Loading {MODEL_NAME} (multiplier={MULTIPLIER})")
    print(f"{'=' * 50}")
    ...
    print(
        f"Configuration: KD + unweighted CE + student {STUDENT_INPUT_SIZE} strong augmentation "
        f"with {TEACHER_INPUT_SIZE} full-image teacher supervision"
    )
    ...
    ckpt_path = os.path.join(cfg.models_dir, f"{MODEL_NAME}_fp32_kd.pth")
    ...
    test_metrics = test(
        ...
        device=device,
        model_type=f"{MODEL_NAME}_fp32_kd",
        bootstrap=True,
    ...
    report = {
        ...
        "student_resolution": STUDENT_INPUT_SIZE,
        "teacher_resolution": TEACHER_INPUT_SIZE,
        "input_size": [1, 3, STUDENT_INPUT_SIZE, STUDENT_INPUT_SIZE],
