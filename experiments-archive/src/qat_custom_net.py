"""
Canonical KD-QAT fine-tuning for the revived CustomSmallNet branch.

This mirrors the active FP32 custom-net baseline as closely as possible:
- teacher: upgraded ResNet18 KD teacher (`resnet18_from_resnet50_fp32_kd.pth`)
- student init: canonical FP32 custom-net checkpoint (`custom_net_m3_fp32_kd.pth`)
...
The planned experiment variable is quantization bit width.
"""
...
from utils.quant_custom_net import QuantCustomSmallNet, load_fp32_weights, model_tag
...
def main(cfg: DictConfig) -> None:
    ...
    print("\n" + "=" * 50)
    print(f"Creating QuantCustomSmallNet [{tag}]")
    print("=" * 50)
    student = QuantCustomSmallNet(
    ...
    if warm_start_checkpoint:
        ...
        student_init_checkpoint = os.path.join(cfg.models_dir, f"{MODEL_NAME}_fp32_kd.pth")
    ...
    missing, unexpected = load_fp32_weights(student, student_init_checkpoint)
    non_quant_missing = [
        key
        for key in missing
        if not any(
    ...
    print(
        f"Configuration: KD-QAT + student {STUDENT_INPUT_SIZE} strong augmentation "
    ...
    logname = os.path.join(cfg.results_dir, f"qat_custom_net_{tag}_log.csv")
    ...
    qat_ckpt_path = os.path.join(cfg.models_dir, f"{MODEL_NAME}_{tag}_qat.pth")
    ...
    test_metrics = test(
        ...
        device=device,
        model_type=f"{MODEL_NAME}_{tag}_qat",
        bootstrap=True,
    ...
    report_path = os.path.join(cfg.results_dir, f"qat_custom_net_{tag}_report.json")
