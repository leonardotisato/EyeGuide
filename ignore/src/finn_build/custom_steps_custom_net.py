...
Based on custom_steps_resnet18.py, simplified:
  ...
  - Has GlobalAvgPool head → InferGlobalAccPoolLayer, plus extra scalar/flatten cleanup
...
def step_custom_net_streamline(model: ModelWrapper, cfg: DataflowBuildConfig) -> ModelWrapper:
    ...
    # Keep scalar Muls separate from the first FC path. Absorbing them into the
    # MatMul weights can turn the quantized FC weights into FLOAT32, which then
    # breaks MVAU HLS template matching during step_hw_ipgen.
    #
    # This mirrors the lesson from the active test_resnet flow: correctness-
    # preserving scalar absorption around GlobalAccPool/Flatten is not worth it
    # if it destroys quantized FC weight typing.
    model = model.transform(Streamline())

    # The CustomSmallNet head uses GlobalAveragePool followed by a scalar Mul,
    # Flatten and FC. These explicit reorders are needed to push the post-GAP
    # scale forward so it can be absorbed into the following threshold instead
    # of being folded into the FC weights as FLOAT32.
...
def step_custom_net_to_hw(model: ModelWrapper, cfg: DataflowBuildConfig) -> ModelWrapper:
    ...
    to_hw_transformations = [
        ...
        AbsorbConsecutiveTransposes(),
        InferGlobalAccPoolLayer(),
        # InferGlobalAccPoolLayer introduces the problematic tail:
        #   GlobalAccPool -> Mul -> Flatten -> MatMul -> MultiThreshold
        # Re-run the scalar-move passes *after* this transform so the positive
        # GAP scale is absorbed into the following thresholds instead of into
        # the FC weights (which would turn TW=float and break MVAU HLS).
        MoveScalarLinearPastInvariants(),
        MoveScalarMulPastMatMul(),
        CollapseRepeatedMul(),
        AbsorbMulIntoMultiThreshold(),
